...
@MediumTest
class CheeseListViewModelTest {

    @Test
    fun showsCheeses() {
        val (_, _, repository) = createDummyRepository()
        val viewModel = CheeseListViewModel(ApplicationProvider.getApplicationContext(), repository)
        assertThat(viewModel.cheeses.observedValue()).isNotEmpty()
    }
